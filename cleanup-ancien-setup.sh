...
for d in "$HOME/.config/gtk-3.0" "$HOME/.config/gtk-4.0"; do
    ...
    grep -q "generated by matugen" "$d/gtk.css" 2>/dev/null || { skip "$d/gtk.css is not matugen's"; continue; }
    if [ -f "$d/gtk.css.avant-post-install" ]; then
        [ "$DRY" = "0" ] && mv "$d/gtk.css.avant-post-install" "$d/gtk.css"
        ok "$d/gtk.css restored from the pre-serpantinum backup"
    else
        drop "$d/gtk.css"
    fi
done
